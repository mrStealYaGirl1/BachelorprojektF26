...
static void ble_tx_task(void *arg)
{
    ...
    while (1)
    {
        if (xQueueReceive(s_tx_q, &item, portMAX_DELAY) == pdTRUE)
        {
            ...
            {
                rc = ble_manager_notify_swing_meta_rc(&item.payload.meta);

                if (rc != 0)
                {
                    notify_fail_count++;
                    ESP_LOGW(TAG,
                             "META notify failed rc=%d event=%u",
                             rc,
                             item.payload.meta.event_id);
                }
                else
                {
                    notify_ok_count++;
            ...
            {
                rc = ble_manager_notify_imu_pkt_rc(&item.payload.imu);

                if (rc != 0)
                {
                    notify_fail_count++;
                    ESP_LOGW(TAG,
                             "IMU notify failed rc=%d event=%u seq_start=%u sample_count=%u",
                             rc,
                             item.payload.imu.event_id,
                             item.payload.imu.samples[0].seq,
            ...
            {
                ESP_LOGW(TAG, "Unknown TX packet_type=%u", item.packet_type);
            }
...
/* =========================================================
   START FÆLLES TX TASK
========================================================= */
void ble_manager_start_tx_task(void)
{
    if (s_tx_q != NULL) return;

    s_tx_q = xQueueCreate(128, sizeof(ble_tx_item_t));
    configASSERT(s_tx_q);

    xTaskCreate(ble_tx_task, "ble_tx", 4096, NULL, 6, &s_tx_task_handle);
}
...
{
    if (!s_tx_q || !pkt) return false;

    ble_tx_item_t item;
    memset(&item, 0, sizeof(item));

    item.packet_type = BLE_PKT_TYPE_IMU;
    item.payload.imu = *pkt;

    return (xQueueSend(s_tx_q, &item, pdMS_TO_TICKS(50)) == pdTRUE);
}
